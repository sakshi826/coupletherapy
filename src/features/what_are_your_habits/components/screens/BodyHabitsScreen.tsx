...
const BodyHabitsScreen = ({ onNext }: { onNext: () => void }) => {
  ...
  const options = (typeof t !== "undefined" ? t : (k) => k)('body_options', { returnObjects: true }) as string[];
      ...
      <div className="w-full max-w-lg space-y-8">
        <header className="space-y-4">
          <div className="flex items-center gap-2 text-primary font-bold text-xs uppercase tracking-widest">
            <Sparkles size={14} />{(typeof t !== "undefined" ? t : (k) => k)("physical_health")}</div>
          <h2 className="text-4xl font-extrabold text-slate-900 leading-tight">{(typeof t !== "undefined" ? t : (k) => k)('body_title')}</h2>
          <p className="text-slate-500 text-base font-medium leading-relaxed">{(typeof t !== "undefined" ? t : (k) => k)('body_subtitle')}</p>
        </header>
